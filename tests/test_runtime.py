...
def test_bundled_playwright_browsers_path_returns_none_when_not_frozen(tmp_path: Path) -> None:
    assert bundled_playwright_browsers_path(
        frozen=False,
        executable=str(tmp_path / "s2a"),
    ) is None


def test_bundled_playwright_browsers_path_prefers_playwright_browsers_dir(tmp_path: Path) -> None:
...
def test_bundled_playwright_browsers_path_uses_meipass_when_needed(tmp_path: Path) -> None:
